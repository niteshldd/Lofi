...
def train(dataset, model, name, train_epoch):
    ...
    def compute_loss(data, label):
        ...
        if name == "lyrics2lofi":
            input = pack_padded_sequence(
                embeddings, embedding_lengths, batch_first=True, enforce_sorted=False)
            pred_chords, pred_notes, pred_tempo, pred_key, pred_mode, pred_valence, pred_energy, kl = \
                model(input, max_num_chords, sampling_rate_chords,
        ...
        def compute_mask(max_length, curr_length):
            arange = torch.arange(max_length, device=device).repeat(
                (chords_gt.shape[0], 1)).permute(0, 1)
        ...
        loss_melody = torch.masked_select(
            loss_melody_notes, mask_melody).mean()
        ...
        loss_total = loss_chords + loss_kl + loss_melody + loss_tempo + \
            loss_key + loss_mode + loss_energy + loss_valence

        tp_chords = torch.masked_select(pred_chords.argmax(
            dim=2) == chords_gt, mask_chord).tolist()
        tp_melodies = torch.masked_select(pred_notes.argmax(
            dim=2) == notes_gt, mask_melody).tolist()
    ...
    for epoch in range(EPOCH):
        ...
        if TEACHER_FORCE:
            ...
            sampling_rate_melodies = sampling_rate_at_epoch(
                epoch - MELODY_EPOCH_DELAY)

        print(
            f"Scheduled sampling rate: C {sampling_rate_chords}, M {sampling_rate_melodies}")
        ...
        ep_train_loss_melody = sum(
            ep_train_losses_melodies) / len(ep_train_losses_melodies)
        ep_train_loss_kl = sum(ep_train_losses_kl) / len(ep_train_losses_kl)
        ep_train_chord_acc = (sum(ep_train_tp_chords) /
                              len(ep_train_tp_chords)) * 100
        ep_train_melody_acc = (sum(ep_train_tp_melodies) /
                               len(ep_train_tp_melodies)) * 100

        ep_val_loss_chord = sum(ep_val_losses_chords) / \
            len(ep_val_losses_chords)
        ep_val_loss_melody = sum(ep_val_losses_melodies) / \
            len(ep_val_losses_melodies)
        ep_val_loss_kl = sum(ep_val_losses_kl) / len(ep_val_losses_kl)
        ep_val_chord_acc = (sum(ep_val_tp_chords) /
                            len(ep_val_tp_chords)) * 100
        ep_val_melody_acc = (sum(ep_val_tp_melodies) /
