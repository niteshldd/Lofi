...
import { getInstrumentName } from './instruments';
...
function displayProduceParams(params: ProduceParams) {
...
function updateProduceParams(): ProduceParams {
  ...
  produceParams.preset = new ProducerPreset({ bassLine: bassPrest, harmony: harmonyPrest, melody: melodyPrest, firstBeatArpeggio: firstBeatArpeggioPrest, firstBeatArpeggioPattern: firstBeatArpeggioPatternPrest });
...
function displayTrack(track: Track) {
  ...
  for (let i = 0; i < track.instrumentNotes.length; i++) {
    ...
    cell.innerHTML = track.instrumentNotes[i].duration.toString();
...
decodeButton.addEventListener('click', async () => {
  ...
  try {
    ...
    const key = document.getElementById('key') as HTMLInputElement;
    decodeParams.key = +key.value

    const mode = document.getElementById('mode') as HTMLInputElement;
    decodeParams.mode = +mode.value

    const bpm = document.getElementById('bpm') as HTMLInputElement;
    decodeParams.bpm = +bpm.value

    const energy = document.getElementById('energy') as HTMLInputElement;
    decodeParams.energy = +energy.value

    const valence = document.getElementById('valence') as HTMLInputElement;
    decodeParams.valence = +valence.value

    const chords = document.getElementById('chords') as HTMLInputElement;
    let chords_arr = chords.value.split(',')
    decodeParams.chords = new Array<number>();
    for (let i = 0; i < 8; i++) {
      decodeParams.chords.push(+chords_arr[i])
    }

    const octave = document.getElementById('octave') as HTMLInputElement;
    decodeParams.octave = +octave.value;

    const melodies = document.getElementById('melodies') as HTMLInputElement;
    var melody_arr = melodies.value.split(',')
    decodeParams.melodies = [[0]]
    decodeParams.melodies.pop()
    for (let i = 0; i < 8; i++) {
      ...
      decodeParams.melodies.push(melody_nums)
    }
  } catch (err) {
    console.log(err)
    generateButton.textContent = 'Error!';
